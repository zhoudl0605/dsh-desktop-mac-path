 ...
 *
 * Restore the login-shell PATH (Homebrew, /etc/paths.d, ...) for agent shell
 * commands running inside a GUI-launched DSH host (DSH Desktop, `dsh web`
 * started from Finder/Dock, ...).
 *
...
export const name = "desktop-path";
...
/**
 * Load the desktop-path plugin: restore the login-shell PATH for all
...
export function apply(ctx, config = {}) {
	...
	if (added.length > 0) {
		ctx.logger.info(`desktop-path: prepended to PATH: ${added.join(", ")}`);
	} else {
		ctx.logger.debug("desktop-path: PATH already complete, nothing to add");
	}
}
